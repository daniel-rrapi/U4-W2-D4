...
public class Main {
	...
	public static void main(String[] args) {
		...
		getOrders().forEach((utente, lista) -> System.out.println("Utente: " + utente.getName() + ", " + lista));
		System.out.println("---------------------------------- ES 2 ------------------------------------");
		getTotalPrice().forEach((utente, totale) -> System.out.println("Utente: " + utente + ", " + totale));
		System.out.println("---------------------------------- ES 3 ------------------------------------");
		getMostExpensive().forEach(System.out::println);
		System.out.println("---------------------------------- ES 4 ------------------------------------");
		System.out.println("Average : " + getAveragePrice());
		System.out.println("---------------------------------- ES 5 ------------------------------------");
		getSumForCategory().forEach((category, totale) -> System.out.println("Categoria: " + category + ", "+ totale));

	}
	public static Map<Customer, List<Order>> getOrders() {
		return 	orders.stream().collect(Collectors.groupingBy(order -> order.getCustomer()));
	}
	 public static Map<Customer, DoubleSummaryStatistics> getTotalPrice() {
		 return orders.stream().collect(Collectors.groupingBy(Order::getCustomer, Collectors.summarizingDouble(Order::getTotal)));
	}
	public static List<Product> getMostExpensive() {
		return warehouse.stream().sorted(Comparator.comparingDouble(Product::getPrice).reversed()).limit(1).toList();
	}
	public static double getAveragePrice() {
		return orders.stream().collect(Collectors.averagingDouble(Order::getTotal));
	}
	public static Map<String, DoubleSummaryStatistics > getSumForCategory() {
		return warehouse.stream().collect(Collectors.groupingBy(Product::getCategory, Collectors.summarizingDouble(Product::getPrice)));
	}
